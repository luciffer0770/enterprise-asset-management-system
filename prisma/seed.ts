...
async function main() {
  ...
  // Projects (for trolleys / engines)
  let projAlpha = await prisma.project.findFirst({
    where: { tenantId: tenant.id, name: "Project Alpha" },
  });
  if (!projAlpha) {
    projAlpha = await prisma.project.create({
      data: { tenantId: tenant.id, name: "Project Alpha", code: "ALPHA" },
    });
  }
  let projBeta = await prisma.project.findFirst({
    where: { tenantId: tenant.id, name: "Project Beta" },
  });
  if (!projBeta) {
    projBeta = await prisma.project.create({
      data: { tenantId: tenant.id, name: "Project Beta", code: "BETA" },
    });
  }

  // Pools (trolleys) - link to project
  ...
  if (!pool1) {
    pool1 = await prisma.inventoryPool.create({
      data: { tenantId: tenant.id, name: "General Tool Pool", projectId: projAlpha.id },
    });
  } else if (!pool1.projectId) {
    pool1 = await prisma.inventoryPool.update({
      where: { id: pool1.id },
      data: { projectId: projAlpha.id },
    });
  }
  ...
  const emails = [
    ...
    { email: "lab@demo.com", name: "Lab In-Charge", role: "LAB_INCHARGE" as const },
  ...
  console.log(`Seeded: ${assetLimit} assets, demo users (Admin, Lab, Mechanical, Electrical, External), org units, locations, projects, pools`);
}
